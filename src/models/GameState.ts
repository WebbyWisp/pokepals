...
export class GameState {
  ...
  public getPokemonBySpecies(speciesId: number): Pokemon[] {
    return Array.from(this.pokemon.values()).filter((p) => p.species.id === speciesId)
  }
  ...
  public processIdleTime(): number {
    ...
    if (minutesIdle > 0 && this.settings.enableIdleProgression) {
      ...
      if (activePokemon) {
        ...
        const crystalsGenerated = activePokemon.generateCrystals(minutesIdle * this.settings.crystalGenerationRate)
  ...
  public validateSaveData(saveData: unknown): boolean {
    try {
