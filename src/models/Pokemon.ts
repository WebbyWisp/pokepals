...
export class Pokemon {
  ...
  private levelUp(): void {
    this.stats.level++
    this.stats.experienceToNext = this.calculateExperienceToNext(this.stats.level)
  ...
  private checkEvolution(): boolean {
    if (this.species.evolutionLevel && this.stats.level >= this.species.evolutionLevel) {
      return true
    }
    return false
  }
  ...
  public generateCrystals(timeMultiplier: number = 1): number {
    ...
    const crystalsGenerated = Math.floor(baseRate * happinessMultiplier * (1 + friendshipBonus) * timeMultiplier)
  ...
  public static fromJSON(data: Record<string, unknown>): Pokemon {
    const pokemon = new Pokemon(data.species as PokemonSpecies, data.nickname as string | undefined)
    pokemon.id = data.id as string
    pokemon.stats = data.stats as PokemonStats
    pokemon.caughtAt = new Date(data.caughtAt as string)
    pokemon.isActive = data.isActive as boolean
    pokemon.mood = data.mood as 'happy' | 'neutral' | 'sad' | 'excited'
    return pokemon
  }
}
