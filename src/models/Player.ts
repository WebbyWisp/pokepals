...
export class Player {
  ...
  public getTotalPlayTime(): number {
    return Math.floor(
      (Date.now() - this.stats.startDate.getTime()) / (1000 * 60)
    );
  }
  ...
  public static fromJSON(data: any): Player {
    ...
    player.stats = {
      ...data.stats,
      startDate: new Date(data.stats.startDate),
      lastActiveDate: new Date(data.stats.lastActiveDate),
    };

    // Restore achievements
    player.achievements = data.achievements.map((a: any) => ({
      ...a,
      unlockedAt: a.unlockedAt ? new Date(a.unlockedAt) : undefined,
    }));

    // Restore biomes
    player.biomes = new Map(data.biomes);

    player.activePokemonId = data.activePokemonId;
