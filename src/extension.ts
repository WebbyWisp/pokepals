...
export async function activate(context: vscode.ExtensionContext) {
  ...
  try {
    ...
    const provider = vscode.window.registerWebviewViewProvider(SidebarProvider.viewType, sidebarProvider)
...
function registerCommands(context: vscode.ExtensionContext) {
  ...
  const showPokemonCommand = vscode.commands.registerCommand('pokepals.showPokemon', () => {
    const gameState = gameManager.getGameState()
    const activePokemon = gameState.getActivePokemon()

    if (activePokemon) {
      const message = [
        `🐾 ${activePokemon.getDisplayName()} (Level ${activePokemon.stats.level})`,
  ...
  // Reset game command
  const resetGameCommand = vscode.commands.registerCommand('pokepals.resetGame', async () => {
    await gameManager.resetGame()
    statusBarProvider.forceUpdate()
  })

  // Save game command
  const saveGameCommand = vscode.commands.registerCommand('pokepals.saveGame', async () => {
...
function registerEventListeners(context: vscode.ExtensionContext) {
  ...
  // Track file saves
  const fileSaveListener = vscode.workspace.onDidSaveTextDocument(async (document: vscode.TextDocument) => {
    await gameManager.handleFileSave(document)
    statusBarProvider.forceUpdate()
  })

  // Track file creation
  const fileCreateListener = vscode.workspace.onDidCreateFiles(async (event: vscode.FileCreateEvent) => {
