...
export class GameManager {
  ...
  private startAutoSave(): void {
    if (this.gameState.settings.autoSave) {
      this.saveManager.startAutoSave(this.gameState, this.gameState.settings.autoSaveInterval)
    }
  }

  public async handleTextDocumentChange(document: vscode.TextDocument): Promise<void> {
    if (!this.gameState.isInitialized) {
      return
    }
  ...
  public async handleFileSave(_: vscode.TextDocument): Promise<void> {
    if (!this.gameState.isInitialized) {
      return
    }
    ...
    if (activePokemon) {
      ...
      // Happiness boost
      activePokemon.stats.happiness = Math.min(100, activePokemon.stats.happiness + 2)
    }
  }
  ...
  private showEvolutionNotification(pokemon: Pokemon): void {
    vscode.window.showInformationMessage(
      `🎉 ${pokemon.getDisplayName()} is ready to evolve! (Level ${pokemon.stats.level})`
    )
  }
  ...
  public async resetGame(): Promise<void> {
    ...
    if (confirm === 'Reset') {
      ...
      await this.saveManager.saveGame(this.gameState)
      vscode.window.showInformationMessage('Pokémon Pals progress has been reset')
    }
  }
