...
import type { SaveData } from '../models/GameState'
...
export class SaveManager {
  ...
  public async loadGame(): Promise<GameState | null> {
    try {
      ...
      let saveData = this.context.globalState.get<SaveData>(SaveManager.saveKey)

      // If not found, try backup from workspace state
      if (!saveData) {
        saveData = this.context.workspaceState.get<SaveData>(SaveManager.backupKey)
      }
      ...
      if (saveData) {
        ...
        if (gameState.validateSaveData(saveData)) {
          ...
          vscode.window.showWarningMessage('Save data appears corrupted. Starting new game.')
        }
      }
      ...
      vscode.window.showErrorMessage('Failed to load Pokémon Pals progress. Starting new game.')
      return null
    }
  }
  ...
  public async exportSave(): Promise<string | null> {
    try {
      const saveData = this.context.globalState.get<SaveData>(SaveManager.saveKey)
      if (saveData) {
  ...
  public async clearSave(): Promise<boolean> {
    try {
      ...
      await this.context.workspaceState.update(SaveManager.backupKey, undefined)
  ...
  public startAutoSave(gameState: GameState, intervalMinutes: number = 5): void {
    this.stopAutoSave()
  ...
  public async migrateData(oldVersion: string, newVersion: string, saveData: SaveData): Promise<SaveData> {
  ...
  public getSaveInfo(): {
    ...
    const saveData = this.context.globalState.get<SaveData>(SaveManager.saveKey)
    const backupData = this.context.workspaceState.get<SaveData>(SaveManager.backupKey)
